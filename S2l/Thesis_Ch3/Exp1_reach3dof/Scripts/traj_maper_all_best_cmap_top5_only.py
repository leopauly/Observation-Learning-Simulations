...
for i in range(int(run)):
        ...
        for episode in  range(num_episodes):
                traj_x_per_episode_=np.array(np.loadtxt('step_man_x_pos_run_'+str(i)+'_eps_'+str(episode)+'.txt', unpack=True))
                traj_x_per_episode=traj_x_per_episode_[1:len(traj_x_per_episode_)]
                traj_x_all.append(traj_x_per_episode)

        for episode in  range(num_episodes):
                traj_y_per_episode_=np.array(np.loadtxt('step_man_y_pos_run_'+str(i)+'_eps_'+str(episode)+'.txt', unpack=True))
                traj_y_per_episode=traj_y_per_episode_[1:len(traj_y_per_episode_)]
                traj_y_all.append(traj_y_per_episode)
...
pos_x_temp=0
pos_y_temp=0
traj_x_per_episode_=0
traj_x_per_episode=0
traj_y_per_episode_=0
traj_y_per_episode=0
pos_x=0
pos_x=0
rewards_=0
rewards=0
...
traj_x_all=np.array(traj_x_all)
traj_y_all=np.array(traj_y_all)
print('Shapes:',rewards_all.shape,pos_x_all.shape,pos_x_all.shape)
print('Traectory array shape:',traj_x_all.shape,traj_y_all.shape)
...
for k in range(len(pos_x_all)):
            ...
            traj_x_episode=traj_x_all[k]
            traj_y_episode=traj_y_all[k]
            for u in  range(len(traj_x_episode)):
                traj_x_point=-(traj_x_episode[u])
                traj_x_converted=(traj_x_point-(-2.2))/ (2.2-(-2.2))* (640-0)+0
                
                traj_y_point=-(traj_y_episode[u])
                traj_y_converted=(traj_y_point-(-.62)) / (2.1-(-.62)) * (0-(-360)) + (-360) #pos_y_converted= (pos_y_-(-1)) / (2.5-(-1)) * (0-(-360)) + (-360
                
                color_value=cm.jet(rewards_norm_all[k]) 
                
                color_value_rgb=(255*color_value[2],255*color_value[1],255*color_value[0]) # (0,0,255*rewards_norm_all[k]) 
                cv2.circle(env_img,(int(traj_x_converted),int(-traj_y_converted)),4,color_value_rgb,-1)
'''
##----------------------------------------------------------------------------------------------------##       
...
for k in range(len(pos_x_all)):
            ...
            for u in  range(len(traj_x_episode)):
                ...
                color_value=cm.jet(.33) 
                color_value_rgb=(255*color_value[2],255*color_value[1],255*color_value[0]) 
                cv2.circle(env_img,(int(traj_x_converted),int(-traj_y_converted)),4,color_value_rgb,-1)


rewards_norm_all_sorted_idx=np.argsort(rewards_all)[::-1]
for k in rewards_norm_all_sorted_idx[0:top_best]:
...
cv2.imwrite('env_img_trajmap_all_best_cmap_top10_only.png',env_img)
cv2.waitKey()
