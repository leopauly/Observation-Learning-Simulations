...
nb_classes=2 
feature_size=4608#(vggnet output) #8192(action vector size)
#frame_feature_size=
...
class Vid_Feature:
    ...
    def __init__(self):
        ...
        with self.g.as_default():
            ...
            self.writer=tf.summary.FileWriter("./logdir",self.g)
    ...
    def feature_extractor(self,vid_np):
        ...
        examine_graph=self.g
        examine_ops=examine_graph.get_operations()
        with open('examine_graph.txt','w') as f: 
            for i in examine_ops:
                f.write(str(i))
            f.close()
        print([x for x in self.g.get_operations() if x.type == "Placeholder"])
        exit()
        #print(self.g.as_graph_def())
        #print([n.name for n in  self.g.as_graph_def().node])
        #print(tf.contrib.graph_editor.get_tensors(self.g))
        #print([n.name for n in  self.g.as_graph_def().node if isinstance(n,tf.Tensor)])
        #exit()

        
        #f_v_summary = self.sess.graph.get_tensor_by_name('flatten_1/Reshape:0')
        #self.conv_out_summary = tf.summary.scalar('conv_out_summary',tf.reduce_mean(f_v_summary,[-1])) 

        f_v_summary = self.sess.graph.get_tensor_by_name('flatten_1/Reshape:0')
        self.conv_out_summary = tf.summary.histogram('conv_out_summary',tf.squeeze(f_v_summary)) 
        self.conv_out_summary_val=self.sess.run(self.conv_out_summary, feed_dict={'conv1_input:0':self.vid_,'Placeholder:0':self.vid_ })          
        self.writer.add_summary(self.conv_out_summary_val,global_step=self.global_step)
        self.global_step=self.global_step+1

        f_v = self.sess.graph.get_tensor_by_name('Softmax_1:0')  #('flatten_1/Reshape:0')
        self.f_v_val=self.sess.run([f_v], feed_dict={'conv1_input:0':self.vid_,'Placeholder:0':self.vid_ }) #,K.learning_phase(): 0 })
        self.f_v_val=np.array(self.f_v_val)

        print("===========================Checking=============================")
        print('Value extracted from network:',self.f_v_val)
        print('Shape of value from network:',np.array(self.f_v_val).shape)
        print("===========================Checking=============================")
...
def s2l(i_run):
    ...
    for i in range(len(demo_vid_array)):
        plt.imshow(demo_vid_array[i])
        plt.savefig('demo_img'+str(i) +'.png')
